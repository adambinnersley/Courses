...
<div class="row">
    ...
    {foreach $documents as $doc}
        {if $group != $doc.group && $doc.group}{if !$first}</ul></div></div>{/if}<div class="col-md-4 col-sm-6"><div class="card"><div class="card-header bg-primary font-weight-bold">{$doc.group}</div>{if $userDetails.isHeadOffice}<ul class="list-group">{/if}{/if}
