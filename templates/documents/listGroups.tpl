...
<div class="row">
    <div class="col-12">
        <a href="/student/learning/{$courseInfo.url}/course-documents/add" title="Add new group" class="btn btn-success float-right"><span class="fa fa-plus fa-fw"></span> Add group</a>
    </div>
</div>
 <div class="card border-primary">
    <div class="card-header bg-primary">Groups</div>
    {if is_array($docs_groups)}
        <ul class="list-group">
        {foreach $doc_groups as $group}
            <li class="list-group-item">{$group.name}<div class="float-right"><a href="/student/learning/{$courseInfo.url}/course-documents/edit/{$group.id}/" title="Edit group" class="btn btn-warning btn-sm"><span class="fa fa-pencil-alt fa-fw"></span> Edit</a>{if $group.items == 0} <a href="/student/learning/{$courseInfo.url}/course-documents/delete/{$group.id}/" title="Delete group" class="btn btn-danger btn-sm"><span class="fa fa-trash fa-fw"></span> Delete</a>{/if}</div></li>
        {/foreach}
        </ul>
    {else}
        <div class="card-body">
