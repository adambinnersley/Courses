...
class Pupils{
    ...
    public function getPupilsOnCourse($courseID){
        if(is_numeric($courseID)){
            return $this->db->query("SELECT * FROM ((SELECT CONCAT(`{$this->users_table}`.`firstname`, ' ', `{$this->users_table}`.`lastname`) as `name`, `{$this->users_table}`.`email`, `{$this->users_table}`.`id`, 0 as `is_instructor` FROM `{$this->users_table}` WHERE `{$this->pupils_table}`.`course_id` = :courseid AND `{$this->pupils_table}`.`user_id` = `{$this->users_table}`.`user_id`) UNION (SELECT `{$this->instructors_table}`.`name`, `{$this->instructors_table}`.`email`, `{$this->instructors_table}`.`id`, 1 as `is_instructor` FROM `{$this->users_table}` WHERE `{$this->pupils_table}`.`course_id` = :courseid AND `{$this->pupils_table}`.`is_instructor` = 1 AND `{$this->pupils_table}`.`instructor_id` = `{$this->instructors_table}`.`id`)) ORDER BY `name` ASC;", [':courseid' => intval($courseID)]);
        }
        return false;
    }
