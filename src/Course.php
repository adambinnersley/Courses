...
class Course extends FileUpload{
    ...
    const CONTENT_TABLE = 'course_content';
    const ACCESS_TABLE = 'course_access';
    ...
    public function getUserCourses($userID){
        if(is_numeric($userID)){
            return $this->db->query("SELECT * FROM `".self::COURSES_TABLE."`, `".self::ACCESS_TABLE."` WHERE `user_id` = '".$userID."' AND `".self::COURSES_TABLE."`.`id` = `".self::ACCESS_TABLE."`.`course_id` AND `".self::COURSES_TABLE."`.`active` = 1;");
        }
        return false;
    }
