...
class Submission extends Test{
    ...
    public function addUserAnswer($userID, $userType, $questionID, $userAnswer, $type = 1){
        if($userAnswer){
            ...
            return $this->db->insert(self::USER_TEST_INFO, array('user_id' => intval($userID), 'user_type' => intval($userType), 'question_id' => intval($questionID), 'answer' => $userAnswer, 'answer_type' => intval($type), 'score' => intval($mark['score']), 'marked' => intval($mark['marked'])));
        }
        return false;
    }
    ...
    public function deleteUserAnswers($testID, $userID, $userType){
        $this->db->query("DELETE FROM `".self::USER_TEST_INFO."` USING `".self::USER_TEST_INFO."`, `".self::QUESTIONS_TABLE."` WHERE `".self::USER_TEST_INFO."`.`user_id` = ? AND `".self::USER_TEST_INFO."`.`user_type` = ? AND `".self::USER_TEST_INFO."`.`question_id` = `".self::QUESTIONS_TABLE."`.`question_id` AND `".self::QUESTIONS_TABLE."`.`test_id` = ?;", array($userID, $userType, $testID));
    }
    ...
    public function countSubmissionByCourse($courseID){
        $submission['total'] = $this->db->query("SELECT count(*) FROM `".self::USER_TEST_STATUS."`, `".self::TESTS_TABLE."` WHERE `".self::USER_TEST_STATUS."`.`status` >= ? AND `".self::USER_TEST_STATUS."`.`test_id` = `".self::TESTS_TABLE."`.`test_id` AND `".self::TESTS_TABLE."`.`course_id` = ?;", array(1, $courseID));
        $submission['unmarked'] = $this->db->query("SELECT count(*) FROM `".self::USER_TEST_STATUS."`, `".self::TESTS_TABLE."` WHERE `".self::USER_TEST_STATUS."`.`status` = ? AND `".self::USER_TEST_STATUS."`.`test_id` = `".self::TESTS_TABLE."`.`test_id` AND `".self::TESTS_TABLE."`.`course_id` = ? AND `".self::TESTS_TABLE."`.`self_assessed` = 0;", array(1, $courseID));
        return $submission;
    }
    ...
    public function getTestStatus($testID, $userID, $userType = 1){
        $testStatus = $this->db->select(self::USER_TEST_STATUS, array('user_id' => intval($userID), 'user_type' => intval($userType), 'test_id' => intval($testID)));
        if(!$testStatus){
            return ['status' => 0];
        }
        return $testStatus;
    ...
    public function getStatusInfo($statusID){
        if(is_numeric($statusID)){
            return $this->db->select(self::USER_TEST_STATUS, array('id' => $statusID));
        }
        return false;
    }
    ...
    public function updateTestStatus($testID, $userID, $userType = 1, $testStatus = NULL){
        ...
        if($this->db->count(self::USER_TEST_STATUS, array('user_id' => intval($userID), 'user_type' => intval($userType), 'test_id' => intval($testID)))){
            return $this->db->update(self::USER_TEST_STATUS, array('score' => intval($score), 'status' => intval($status)), array('user_id' => intval($userID), 'user_type' => intval($userType), 'test_id' => intval($testID)));
        }
        else{
            return $this->db->insert(self::USER_TEST_STATUS, array('user_id' => intval($userID), 'user_type' => intval($userType), 'test_id' => intval($testID), 'score' => intval($score), 'status' => intval($status)));
        }
    }
    ...
    public function deleteUserStatus($userID, $userType = 1){
        return $this->db->delete(self::USER_TEST_STATUS, array('user_id' => intval($userID), 'user_type' => intval($userType)));
    }
        ...
        if(is_array($scores)){
            foreach($scores as $i => $score){
                if(is_numeric($score)){
                    ...
                    $this->db->update(self::USER_TEST_INFO, array('score' => $score, 'marked' => 1, 'feedback' => $questionFeedback), array('id' => $i));
                }
            }
